...
tazf     = Args.[TAZ Layer]
{oboro_in} = RunMacro("TCB Add DB Layers", tazf)
...
// GM for HBW, HBO, NHB, HBSc, HBU, LIGHT, MED, HEAVY, EI_Auto, EI_SU, EI_Comb
     ...
     Opts = null
     Opts.Input.[PA View Set] = {tazf+"|"+oboro_in, oboro_in}
     ...
     Opts.Field.[Prod Fields] = {oboro_in+".HBW_bal", oboro_in+".HBO_bal", oboro_in+".NHB_bal",
                                 oboro_in+".HBsc_bal", oboro_in+".HBU_bal",
                                 oboro_in+".LIGHT", oboro_in+".MED", oboro_in+".HEAVY",
                                 oboro_in+".EI_Auto_P", oboro_in+".EI_SU_P", oboro_in+".EI_Comb_P"}
     Opts.Field.[Attr Fields] = {oboro_in+".ATTHBW_bal", oboro_in+".ATTHBO_bal", oboro_in+".ATTNHB_bal",
                                 oboro_in+".ATTHBsc_bal", oboro_in+".ATTHBU_bal", 
                                 oboro_in+".ATTLIGHT", oboro_in+".ATTMED", oboro_in+".ATTHEAVY",
                                 oboro_in+".EI_Auto_A_bal", oboro_in+".EI_SU_A_bal", oboro_in+".EI_Comb_A_bal"}
...
if(feedback_iteration=1)  then do
     ...
     Opts.Input.[PA View Set] = {tazf+"|"+oboro_in, oboro_in, "Selection", "Select * where nz(ExSta)>0"}
     ...
     Opts.Field.[P Core Fields] = {oboro_in+".EE_Auto_P", oboro_in+".EE_SU_P", oboro_in+".EE_Comb_P"}
     Opts.Field.[A Core Fields] = {oboro_in+".EE_Auto_P", oboro_in+".EE_SU_P", oboro_in+".EE_Comb_P"}
...
//          Error Trap for tlcl
            if(tlcl=0) then do
               lid  =link_lyr.ID
               ShowMessage("** Link Attribute Error 2 ln sp** : link "+i2s(lid)+" has null or 0 Two-Lane Highway Class, correct Master Network and Re-run")
               return(null)
            end
...
// error trap
        if(SL=0)  then do
          lid = link_lyr.ID
          ShowMessage("** Link Attribute Error ramp ** LINKID="+i2s(lid)+" [PostedSpeed] is null or 0, correct Master Network and Re-run")
          return(null)
        end
        if DIR > (-1) and ABL=0 then do
          lid  =link_lyr.ID
        ...
        if(rv=0)  then do
          lid = link_lyr.ID
          ShowMessage("** Link Attribute Error ramp ** LINKID="+i2s(lid)+" RAMP is null or 0, correct Master Network and Re-run")
          return(null)
        end
...
// get highest internal TAZ
   ...
   SetLayer(oboro_in)
   TAZvw=getview()
   qry = "Select * where nz(ID)<>null"    // "Select * where EXT=null"
   n = SelectByQuery("Internal", "Several", qry,)
...
SetLayer(link_lyr)
 ...
 if(myselect=1) then do
    ...
    qry1 = "Select * where nz("+cfld+") >0 & (In_Network=1) & (CO_NUMBER=30)"
 end
...
endMacro
*/
Macro "purp9t4"  // aggregate AirSage Trip Tables to 9 purposes and 4 time periods: AM, MD, PM NT
  // create empty output matrix
...
Macro "purp3t4"  // aggregate AirSage Trip Tables to 3 purposes and 4 time periods: AM, MD, PM NT, in the P-A Direction
  // create empty output matrix
...
Macro "TLFD2"  // calculate TLFD for AirSage Trip Tables
    RunMacro("TCB Init")
    ocores={"HBW","HBO","NHB"}
    t5={"AM","MD","PM","NT","Day"}
...
Macro "gmcal"       // calibrate GM's using both gammas and FF methods
    RunMacro("TCB Init")
